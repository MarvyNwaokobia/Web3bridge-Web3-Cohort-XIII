...
import { RolesRegistry, TokenGatedDAO } from "../typechain-types";

describe("Token Gated DAO", function () {
  let nftContract: RolesRegistry;
  ...
  describe("Role Management", function () {
    it("Should grant and check roles correctly", async function () {
      ...
      expect(await nftContract.userHasRole(admin.address, adminRole)).to.be
        .true;
      expect(await nftContract.userHasRole(member.address, memberRole)).to.be
        .true;
    });
